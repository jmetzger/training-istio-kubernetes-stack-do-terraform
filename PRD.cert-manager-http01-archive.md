...
### 12.1 Terraform Dependency Cycle bei Droplet Deletion

**Problem:** Beim `terraform destroy` oder `terraform apply` tritt ein Zyklus-Fehler auf:

```
Error: Cycle: null_resource.pre_destroy_lb_cleanup, null_resource.pre_destroy_helm_cleanup, digitalocean_droplet.k8s_nodes
```

**Ursache:**

Die Implementierung der Pre-Destroy Hooks in `main.tf` erzeugt einen zirkulären Abhängigkeitsgraphen:

```
digitalocean_droplet.k8s_nodes (Zeile 101-120)
  ├─ depends_on: null_resource.pre_destroy_helm_cleanup (Zeile 117)
  └─ depends_on: null_resource.pre_destroy_lb_cleanup (Zeile 118)

null_resource.pre_destroy_helm_cleanup (Zeile 52-69)
  └─ triggers.control_plane_ip = digitalocean_droplet.k8s_nodes[0].ipv4_address (Zeile 54)
     └─ Erzeugt implizite Abhängigkeit: k8s_nodes → pre_destroy_helm_cleanup

null_resource.pre_destroy_lb_cleanup (Zeile 72-96)
  └─ depends_on: null_resource.pre_destroy_helm_cleanup (Zeile 73)
```

**Zyklus:**
1. `k8s_nodes` hängt von `pre_destroy_helm_cleanup` ab (explicit depends_on)
2. `pre_destroy_helm_cleanup` hängt von `k8s_nodes[0].ipv4_address` ab (trigger)
3. `pre_destroy_lb_cleanup` hängt von `pre_destroy_helm_cleanup` ab (depends_on)
4. `k8s_nodes` hängt von `pre_destroy_lb_cleanup` ab (explicit depends_on)

**Konflikt:**
- Die `depends_on` Deklaration in `k8s_nodes` (Zeile 116-119) sagt: "Lösche erst die Cleanup-Hooks, dann die Droplets"
- Der `trigger` in `pre_destroy_helm_cleanup` (Zeile 54) erzeugt: "Lösche erst die Droplets, dann die Cleanup-Hooks"
- → Widersprüchliche Anforderungen = Cycle Error

**Lösung:**

Entferne die `depends_on` Abhängigkeit von den Droplets auf die Cleanup-Hooks:

```hcl
# main.tf - Zeile 101-120
resource "digitalocean_droplet" "k8s_nodes" {
  ...
  # WICHTIG: depends_on ENTFERNT, um Cycle zu vermeiden
  # Die Cleanup-Reihenfolge wird durch trigger in null_resources gesteuert
}
```
...
1. **Destroy-Ablauf:**
   - Terraform erkennt, dass `k8s_nodes[0].ipv4_address` sich ändert (destroy)
   - Trigger in `pre_destroy_helm_cleanup` feuert → Helm Cleanup läuft
   - `pre_destroy_lb_cleanup` depends_on `pre_destroy_helm_cleanup` → LB Cleanup läuft danach
   - Droplets werden gelöscht

2. **Reihenfolge garantiert durch:**
   - `trigger` in `pre_destroy_helm_cleanup` (reagiert auf Droplet-Änderung)
   - `depends_on` in `pre_destroy_lb_cleanup` (Helm vor LB)
   - Kein explizites `depends_on` in `k8s_nodes` nötig
...
# Erwartete Ausgabe: Kein Cycle Error
# Destroy-Reihenfolge:
# 1. pre_destroy_helm_cleanup (via trigger)
# 2. pre_destroy_lb_cleanup (via depends_on)
# 3. digitalocean_droplet.k8s_nodes

# Terraform Apply testen
terraform apply -auto-approve

# Terraform Destroy testen
terraform destroy -auto-approve
```

**Wichtige Hinweise:**

- Der `trigger` in `null_resource` ist ausreichend, um die Cleanup-Reihenfolge zu steuern
- Ein zusätzliches `depends_on` in den Droplets ist nicht nur unnötig, sondern erzeugt einen Zyklus
- Terraform kann die Abhängigkeiten automatisch durch `trigger` erkennen
- Die Löschung erfolgt trotzdem in der richtigen Reihenfolge: Helm → LB → Droplets

**Status:** In Bearbeitung im Branch `feature/fix-droplet-deletion-timeout`
